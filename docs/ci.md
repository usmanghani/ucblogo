...
The text editor saves each edit in sessionStorage, scoped to the current browser tab. Refresh restores the exact source, including an intentionally empty editor. It does not execute restored code automatically. `Clear saved session` asks before clearing the editor and removes only this program's storage key. Refresh after clearing starts with the default example. Storage failures are shown beside the clear button. This draft is not a substitute for saving/exporting programs, and does not persist across closed browser sessions.
